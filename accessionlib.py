...
class LogicalDB:
    ...
    ACC_LogicalDB table in the MGI database.
        ...
        def __init__(self, logicaldbname, logicaldbkey, description, 
                ...
                """ constructor
                # requires: 
                #    logicaldbname: name of the LogicalDB (str..
                #    logicaldbkey: primary key/identifier of the logicaldb (integer).
                #    description: textual description of the logicaldb (str..
        ...
        def getName(self):
                ...
                # requires: nothing
                # effects: none
                # returns: the name of this object (str.
                # exceptions: none
                """
                return self.name
        ...
        def getDescription(self):
                ...
                # requires: nothing
                # effects: none
                # returns: the textual description of this object (str..
                # exceptions: none
                """
...
class LogicalDBTable:
        ...
        def getDBbyKey(self, LogicalDBKey):
                ...
                # exceptions: none.
                """
                if LogicalDBKey in self.DBbyKey:
        ...
        def getDBbyName(self, LogicalDBName):
                ...
                # exceptions: none.
                """
                if LogicalDBName in self.DBbyName:
...
class ActualDB:
    ...
    table in the MGI database.
        ...
        def __init__(self, actualdbname, actualdbkey, logicaldbname, logicaldbkey,
                ...
                """ constructor
                # requires: 
                #    actualdbname: name of the ActualDB (str..
                ...
                #               the multiple CGI arguments (str., or None.
                # effects: initializes object with provided parameters. 
                # exceptions: none
                """
        ...
        def getURL(self):
                ...
                # requires: nothing
                # effects: none
                # returns:  URL associated with this DB (str..
                # exceptions: none
                """
                return self.url
        ...
        def getDelimiter(self):
                ...
                # requires: nothing
                # effects: none
                # returns: delimiter (str. used to separate multiple arguments to
...
class ActualDBTable:
        ...
        def loadtable(self):
                ...
                for result in results:
                        ...
                        if logicaldbname not in self.DBbyName:
                                self.DBbyName[logicaldbname] = {}
                        self.DBbyName[logicaldbname][actualdbname] = adb 
                        if logicaldbkey not in self.DBbyKey:
        ...
        def getDBbyKeys(self, LogicalDBKey, ActualDBKey=None):
                ...
                if ActualDBKey is None:
                        if LogicalDBKey in self.DBbyKey:
                                rl = []
                                for adbkey in list(self.DBbyKey[LogicalDBKey].keys()):
        ...
        def getDBbyNames(self, LogicalDBName, ActualDBName=None):
                ...
                if ActualDBName is None:
                        if LogicalDBName in self.DBbyName:
                                rl = []
                                for adbname in list(self.DBbyName[LogicalDBName].keys()):
...
def get_source( LogicalDB, Acc_ID, actualDBTable):
        ...
        # Acc_ID - A str.or list of str. that are accession IDs in MGD.
        ...
        for adb in adblist:  # note these are objects, not names now
                if adb.getLogicalDBName() == 'MGI':
                        ...
                        if url is None:
                                ...
                                url = re.sub('@@@@', str.joinfields(Acc_ID, delimiter), url)
                                html = '<A HREF="%s">%s</A>' % (url, adbname)
                links.append( html )
        s = str.joinfields( links, ', ' )
        s = '(' + s + ')'
...
def get_links(_Object_key, MGIType=None, LogicalDB=None, view='Acc_View',
        ...
        """Returns a Python dictionary of accession IDs for given _Object_key.
        #
        # The dictionary keys are accession IDs, the values are HTML str.
...
def get_Accession_key(_Object_key, MGIType=None, LogicalDB=None,
        ...
        #	LogicalDB -- A str.representing the LogicalDB ('MGI',
        #		'Sequence DB'...)
...
def build_sql( search, table=None ):
        ...
        where_list = []
        for prefixPart in list(id.keys()):
                sets = []
        ...
        where = '(%s)' % str.joinfields( where_list, ' or ' ) \
                + ' and %sprivate = 0' % pre
        return where
...
def get_MGIType_key( MGIType ):
        ...
        #
        # Requires:
        #	MGIType -- A str.representing the object type ('Marker',
        #		'Segment'...)
        #
        """
...
def get_LogicalDB( _LogicalDB_key ):
        """Returns the LogicalDBfor a given _LogicalDB_key.
        #
        # Returns a str.('MGI', 'Sequence DB', etc.) if valid, None
        # otherwise.
        #
        # Requires:
...
# simple_accnum   : <str.w/o ".."  "+" and spaces>
# 
...
#   independently.
#
# The str.to parse is kept in the global CurStringToParse str.variable.
...
CurStringToParse = ""	# the current str.to parse, shared by the parsing
                        #  routines below

DefaultErrorMsg = "Error:  Invalid accession ID query."
                        # the default error msg to use for syntax errors

def parse_expr (s	# str. the query expression to parse
                ):
    ...
    for sae in single_accnum_exprs:		# for each single_accnum_expr
        ...
        if ( prefix not in retval ):	# new prefix
...
def simple_accnum (
    ...
    #		prefix (str.	- the prefix part of the 1st accnum in 's'
...
def tail ( vallist	# list of numeric parts and range pairs seen so far.
    ...
    #		CurStringToParse.
    # Returns: None if no error, str.holding error msg if syntax error.
...
def number (
    ...
    #	       str.w/ errmsg if number not found.
    # Assumes: nothing
    # Effects: removes the "<number>" from CurStringToParse.
